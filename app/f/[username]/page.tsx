...
function StatPill({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col items-center px-5 py-3 rounded-2xl bg-white border border-gray-100 shadow-sm">
  ...
  if (!profile) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-6">
      <div className="text-center max-w-sm">
        ...
        <p className="text-gray-500 mb-6">No founder at <span className="font-mono text-gray-800">/f/{username}</span></p>
        <Link href="/" className="px-6 py-3 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-600 transition-colors">Go home</Link>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
        ...
        <div className="bg-emerald-50 border-b border-emerald-200 px-4 py-2.5">
          <div className="max-w-3xl mx-auto flex items-center justify-between gap-4 text-sm">
            <span className="text-emerald-700 font-medium">
              👁 This is how connectors see your profile
            </span>
            <div className="flex items-center gap-3">
              <Link href="/builder" className="text-emerald-600 hover:text-emerald-700 transition-colors underline underline-offset-2 text-xs">
                Edit profile
              </Link>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            {isOwner ? (
              <>
                ...
                <Link href="/builder" className="px-4 py-2 bg-gray-100 border border-gray-200 text-gray-700 text-xs sm:text-sm font-medium rounded-full hover:bg-gray-200 transition-all">
                  Edit Profile
                </Link>
              </>
            ) : (
              <>
                ...
                {!isLoggedIn && (
                  <Link href="/signup" className="px-4 py-2 bg-emerald-500 text-white text-xs sm:text-sm font-semibold rounded-full hover:bg-emerald-600 transition-all">
                    Create yours
                  </Link>
                )}
              </>
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-12">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8 mb-5">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-emerald-200 bg-emerald-50 text-emerald-700 text-xs font-medium mb-4">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
            Startup Profile
          </div>
          <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 mb-2 tracking-tight">{profile.company_name}</h1>
          <p className="text-xl text-gray-500 font-light leading-relaxed mb-5">{profile.one_liner}</p>
        ...
        {/* The Ask */}
        <div className="p-6 sm:p-8 rounded-2xl bg-gradient-to-br from-emerald-500 to-green-600 mb-5 shadow-md">
          <h2 className="text-xs font-semibold text-emerald-100 uppercase tracking-wider mb-3 flex items-center gap-2">
          ...
          {needsList.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-5">
              {needsList.map(n => (
                <span key={n} className="px-3 py-1.5 rounded-full text-sm font-medium bg-white/20 text-white border border-white/30">{n}</span>
              ))}
            </div>
          )}
        </div>
        ...
        {/* Traction */}
        <div className="p-6 rounded-2xl bg-white border border-gray-100 shadow-sm mb-4">
          <h2 className="text-xs font-semibold text-emerald-600 uppercase tracking-wider mb-3">Traction</h2>
          <p className="text-gray-700 leading-relaxed whitespace-pre-line">{profile.traction}</p>
        </div>

        {/* Team */}
        {profile.team && (
          <div className="p-6 rounded-2xl bg-white border border-gray-100 shadow-sm mb-4">
            <h2 className="text-xs font-semibold text-emerald-600 uppercase tracking-wider mb-3">Team</h2>
            <p className="text-gray-700 leading-relaxed whitespace-pre-line">{profile.team}</p>
          </div>
        )}
        ...
        {linksArray.length > 0 && (
          <div className="p-6 rounded-2xl bg-white border border-gray-100 shadow-sm mb-6">
            <h2 className="text-xs font-semibold text-emerald-600 uppercase tracking-wider mb-3">Links</h2>
        ...
        {!isLoggedIn && (
          <div className="p-8 rounded-2xl bg-white border border-gray-100 shadow-sm text-center">
            <p className="text-lg font-semibold text-gray-900 mb-2">Create your own startup profile</p>
            <p className="text-gray-500 text-sm mb-5">One link with everything connectors need. Free during beta.</p>
            <Link href="/signup" className="inline-block px-8 py-3 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-600 transition-all">
              Get started — Free
            </Link>
          </div>
        )}
        ...
        {isLoggedIn && !isOwner && (
          <div className="p-6 rounded-2xl bg-white border border-gray-100 shadow-sm text-center">
            <p className="text-gray-500 text-sm mb-3">Looking to make warm intros?</p>
            <Link href="/connectors" className="text-emerald-600 hover:text-emerald-700 text-sm font-medium transition-colors">
              Browse Connectors →
            </Link>
          </div>
        )}
      ...
      {/* Footer */}
      <footer className="border-t border-gray-200 bg-white py-8 px-6 mt-10">
        <div className="max-w-3xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4 text-sm text-gray-400">
          <Link href="/" className="font-semibold text-gray-900 hover:text-emerald-600 transition-colors">Warmchain</Link>
          <div className="flex gap-6">
            <Link href="/about" className="hover:text-gray-700 transition-colors">About</Link>
            <Link href="/connectors" className="hover:text-gray-700 transition-colors">Connectors</Link>
